...
+item.label.substring(idx+lookup.length,item.label.length);}else
...
break;}}
this.field.nextSibling.querySelectorAll('.dropdown-item').forEach((item)=>{item.addEventListener('click',(e)=>{let dataValue=e.target.getAttribute('data-value');this.field.value=e.target.innerText;if(this.options.onSelectItem)
this.options.onSelectItem({value:e.target.getAttribute('data-value'),label:e.target.innerText,kite:e.target});this.dropdown.hide();})});return items.childNodes.length;}}
